#!/usr/bin/env python3
"""Train the GLR transition head on Open-R1 math CoT displacements (HUB-245)."""
...
def main() -> int:
    ...
    print(f"trainable: {n_train} (head {n_head})", flush=True)
    if n_train != n_head or not (3_000_000 < n_head < 5_000_000):
        print(f"FAIL: expected ~4M head-only, got {n_train}", file=sys.stderr)
        return 1
